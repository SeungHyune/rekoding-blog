...
export const PORTFOLIO_PROJECTS: PortfolioProject[] = [
  {
    ...
    achievements: [
      ...
      "네트워크 단절 시 서버 인메모리 기반 대화 맥락 복원을 통한 무중단 세션 재연결 로직 구현",
      "LLM Tool Calling 기반 상태 관리 및 엄격한 서버 측 검증 플로우로 시스템 안정성 확보",
      "LLM Tool Calling 기반 상담 상태 관리와 서버 측 검증 플로우로 정보 누락·중복 단계 전환·조기 예약 완료 방지",
      "30자리 Access Key와 2분 TTL, Redirect URL 구조를 활용한 비회원 보안 진입 인프라 구축",
      "Qdrant(Vector DB) 기반 시맨틱 검색 및 지식베이스 실시간 갱신 구조를 통한 맞춤형 RAG 파이프라인 구현",
    ],
    detailSections: [
      {
        title: "프로젝트 개요",
      ...
      },
      {
        ...
        items: [
          "Gemini Live와 Socket.IO 기반의 실시간 웹소켓 파이프라인을 구축하여, 본인 확인부터 심사 필수 정보 수집, 상담 예약까지의 전 과정을 자동화함으로써 24시간 무중단 상담 응대 체계를 실현했습니다.",
          "네트워크 단절 시 서버 인메모리(In-Memory)에 저장된 대화 맥락(History)과 마지막 AI 응답 상태를 기반으로 무중단 세션 재연결 로직을 구현하여 음성 상담의 연속성을 대폭 개선했습니다.",
        ],
      },
      {
        title:
          "[LLM Tool Calling] AI 대화 상태 관리 및 백엔드 검증 플로우 구축",
        items: [
          "LLM의 Tool Call을 활용해 AI 응답 생성과 동시에 고객 정보 기록, 상담 단계 전환, 예약 완료 처리가 백엔드와 실시간으로 동기화되도록 구현했습니다.",
          "필수 정보 누락, 중복 단계 전환, 조기 예약 완료 등 실시간 대화 중 발생할 수 있는 예외 상황을 방지하기 위해 엄격한 서버 측 검증 아키텍처를 도입하여 시스템 안정성을 확보했습니다.",
        ],
      },
      {
        title:
          "[Authentication / TTL] 외부 제휴사 유입 고객을 위한 보안 진입 플로우 설계",
        items: [
          "외부 제휴사 고객 정보를 기반으로 동적 상담 세션을 생성하고, 30자리 Access Key와 2분 TTL(단기 만료), Redirect URL 구조를 설계하여 별도의 회원가입 없이도 안전하게 상담에 진입할 수 있는 비회원 인증 인프라를 구축했습니다.",
        ],
      },
      {
        title:
          "[Qdrant / Vector DB] RAG 기반 맞춤형 보험 상품 추천 파이프라인 구축",
        items: [
          "고객의 나이, 성별, 관심 보장 조건으로 1차 필터링을 거친 후, Qdrant(Vector DB)의 시맨틱 검색을 통해 고객의 상담 맥락에 부합하는 보험 상품 약관 데이터를 추출하여 AI 추천 로직에 연동했습니다.",
          "관리자 대시보드를 통한 지식베이스(Knowledge Base) 실시간 갱신 구조를 구축하여, 최신 보험 상품 정보가 추천 결과에 즉시 반영되도록 운영 프로세스를 개선했습니다.",
        ],
      },
    ],
  ...
  },
  {
    ...
    description:
      "AI 솔루션 전문 기업 Clevi의 공식 홈페이지 및 자체 관리자 CMS를 기획부터 디자인, 프론트엔드, 서버까지 풀스택으로 단독 개발한 프로젝트",
    ...
    achievements: [
      "Clevi 공식 홈페이지 단독 기획·디자인 및 프론트엔드부터 서버까지 풀스택 개발",
      "Tiptap 기반 WYSIWYG 에디터와 Supabase Storage를 연동한 자체 콘텐츠 관리자 CMS 개발",
      "'개발자 코드 배포'에서 '관리자 직접 수정' 구조로 전환하여 콘텐츠 운영 시 개발자 개입 리소스를 1명 → 0명으로 단축",
      "next-intl 기반 다국어(한국어·영어) 라우팅 아키텍처 설계",
      "API Route를 활용한 JWT 기반 관리자 인증 구현 및 서버 사이드 LLM API 호출을 통한 Key 노출 사전 차단",
    ],
    detailSections: [
      {
        title: "프로젝트 개요",
        items: [
          "AI 솔루션 전문 기업 Clevi의 공식 홈페이지를 단독으로 기획 및 디자인하고, 프론트엔드부터 서버까지 풀스택으로 구축했습니다.",
          "단순한 소개 페이지에 그치지 않고, 비개발자도 기술 자료와 뉴스를 직접 운영할 수 있도록 자체 관리자 CMS를 함께 개발하여 사내 콘텐츠 운영 효율성을 극대화했습니다.",
        ],
      },
      {
        title: "[Next.js / Tiptap] 콘텐츠 관리자 CMS 구축",
        items: [
          "Tiptap 기반 WYSIWYG 에디터와 Supabase Storage를 연동한 관리자 CMS를 구축하여, 비개발자가 기술 자료와 뉴스 콘텐츠를 직접 등록·수정·이미지 교체할 수 있는 운영 환경을 마련했습니다.",
          "콘텐츠 운영 방식을 '개발자 코드 배포 → 반영' 구조에서 '관리자 직접 수정 → 즉시 반영' 구조로 전환하여, 콘텐츠 작업 시 개발자의 개입을 1명에서 0명으로 완벽히 단축했습니다.",
        ],
      },
      {
      ...
      {
        title: "[Security] API Route를 활용한 JWT 인증 및 API Key 은닉",
        items: [
          "JWT 기반 관리자 인증 및 토큰 갱신 플로우를 구현하여 안전한 관리자 환경을 구축했습니다.",
          "클라이언트가 아닌 서버 사이드(API Route)에서 외부 LLM API를 호출하도록 아키텍처를 설계하여, API Key 외부 노출 위험을 원천적으로 차단했습니다.",
        ],
      },
    ],
    tags: ["Next.js", "Tiptap", "Supabase", "next-intl"],
  },
  {
    id: "03",
    title: "마실가실",
